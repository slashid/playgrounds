...
import standard from "./images/lg/standard.jpeg";
import deluxe from "./images/lg/deluxe.jpeg";
import suite from "./images/lg/suite.jpeg";
import presidentialSuite from "./images/lg/presidential-suite.jpeg";
...
export function findRoomById(rooms: Room[], id: string) {
  const room = rooms.find((product) => product.id === id);
...
export function formatRoomPrice({ price: { currency, amount } }: Room) {
  return `${currency}${amount.toFixed(2)} / night`;
}
